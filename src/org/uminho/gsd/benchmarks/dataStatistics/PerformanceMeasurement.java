...
public class PerformanceMeasurement
{
	...
	public static void monitor(double threshold) throws Exception
	{
		...
		try
		{
			...
			TopParser errorGobbler = new TopParser(proc.getErrorStream(),
					"ERROR (top)", threshold);

			TopParser outputGobbler = new TopParser(proc.getInputStream(),
					"OUTPUT", threshold);
	...
	public static void invokeStack(int app_pid, int t_pid)
	{
		...
		try
		{
			String[] cmd = { "jstack", app_pid + "" };

			Runtime rt = Runtime.getRuntime();
			Process proc = rt.exec(cmd);
			// any error message?
			StackParser errorGobbler = new StackParser(proc.getErrorStream(),
					"ERROR (jstack)", 0);

			// any output?
			StackParser outputGobbler = new StackParser(proc.getInputStream(),
					"OUTPUT", t_pid);
			...
			try
			{
				...
				while ((line = br.readLine()) != null)
				{
					...
					if (line.contains("java"))
					{
						...
						double cpu_level = Double
								.parseDouble(info[real_cpu_position].trim());
						int pid = Integer.parseInt(info[real_pid_position]
								.trim());
						if (cpu_level > threshold)
						{
							PerformanceMeasurement.invokeStack(
									PerformanceMeasurement.app_pid, pid);
						}
