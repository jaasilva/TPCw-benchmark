...
public class ConcurrentResultHandler
{
	 ...
	 *            the devision in the data by runs. If -1 all will be inserted
	 *            un the same run;
	 */
	...
	public synchronized void countEvent(String eventType, String event,
			long number)
	{
	...
	public void addResults(ConcurrentResultHandler other_results)
	{
		...
		for (String event_name : new_events.keySet())
		{
			...
			{
				ConcurrentHashMap<String, Long> new_event_count = new_events
						.get(event_name);
				ConcurrentHashMap<String, Long> this_event_count = this.events
						.get(event_name);
