 ...
 * 1.init - The class prepares execution in all benchmarking nodes at the same
 * time <br>
 * 2.deploy the clients - The class deploys several clients that should generate
 * instruction to be executed by a
 * {@linkplain org.uminho.gsd.benchmarks.interfaces.executor.DatabaseExecutorInterface
 * database executor}. <br>
...
public abstract class AbstractWorkloadGeneratorFactory
{
	 ...
	 * 
	 * @param executor
	 *            The benchmark executor with info about the client number among
	 *            others.
	 * @param workloadFile
	 *            The workload file name to be fetched from the configuration
	 *            files, or other place
	 */
	public AbstractWorkloadGeneratorFactory(BenchmarkExecutor executor,
			String workloadFile)
	{
	...
	private void loadFile()
	{
		...
		{
			Logger.getLogger(AbstractWorkloadGeneratorFactory.class.getName())
		...
		finally
		{
			...
			{
				Logger.getLogger(
						AbstractWorkloadGeneratorFactory.class.getName()).log(
						Level.SEVERE, null, ex);
			}
		}

		Map<String, Map<String, String>> map = JsonUtil
				.getStringMapMapFromJsonString(jsonString_r);
		...
		{
			System.out
					.println("[WARNING:] NO REAL WORKLOAD INFO LOADED, YOU SHOULD USE THE Workload TAG");
		}
	 ...
	 * This operation is synchronized between the master and slaves, and should
	 * contain operations that prepare the future clients to start from a specif
	 * point
	 */
	 ...
	 * This method is used by the platform executor to assign a generation
	 * client to each database executor client.
	 * 
	 ...
	 * This method is invoked after all clients finish and it is intended to
	 * allow the user to execute operations over collected data.
	 */
	 ...
	 *            the results collected under this benchmark node.
	 */
	public abstract void finishExecution(List<ResultHandler> collected_results);

	public void setDatabaseFactory(
			AbstractDatabaseExecutorFactory databaseFactory)
	{
