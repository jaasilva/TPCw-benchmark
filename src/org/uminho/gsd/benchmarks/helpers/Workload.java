...
public class Workload
{
	...
	public static void main(String[] args)
	{
		...
		// As the the values for the buy confirm and buy request are not
		// included, there is a second iteration to add the remaining value to
		// the other options.
		for (int i = 0; i < 2; i++)
		{
		...
		// set buy options probabilities. If shopping cart is 2% and the buy
		// request is 0,75% then of each cart there is a 0,41% probability.
		...
		System.out.println("BR:" + buy_request_prob + "  BC:"
				+ buy_confirm_prob);
		...
		for (int i = 0; i < num_operations; i++)
		{
			...
			else
			{
				...
				else if (d < workload_values.get("shoppingCart"))
				{
					...
					{
						// on next operation do a buy request id the probability
						// checks
						shopping = true;
						confirm = false;
					}
		...
		System.out
				.println("OP2 NewProd:" + ((i2 / (operation_num * 1d)) * 100));
		System.out.println("OP3 BestSellers:"
				+ ((i3 / (operation_num * 1d)) * 100));
		System.out
				.println("OP4 PDetail:" + ((i4 / (operation_num * 1d)) * 100));
		System.out.println("OP5 Search:" + ((i5 / (operation_num * 1d)) * 100));
		System.out.println("OP9 ShoppingCart:"
				+ ((i9 / (operation_num * 1d)) * 100));
		System.out.println("OP6 CostumerRegis:"
				+ ((i6 / (operation_num * 1d)) * 100));
		System.out.println("OP10 BRequest:"
				+ ((i10 / (operation_num * 1d)) * 100));
		System.out.println("OP11 BConfirm:"
				+ ((i11 / (operation_num * 1d)) * 100));
		System.out.println("OP7 OrderInqui:"
				+ ((i7 / (operation_num * 1d)) * 100));
		System.out.println("OP8 AdminCofirm:"
				+ ((i8 / (operation_num * 1d)) * 100));
