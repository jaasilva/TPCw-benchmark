...
public class TestClass
{
	...
	// "jdbc:mysql://192.168.82.20:3306/TPCW";
	...
	public static void main(String[] args)
	{
		...
		//
		// try {
		// System.out.println(read_from_ColumnFamily(client, "Address",
		// "streetR]zeGmmEP$secj;Riq.Q=street}CgM=GBOVaw-}#EO_yW}RFII|oF.z._oYaOB[MG!;j+sYorg.uminho.gsd.benchmarks.TPCW_CassandraOM.entities.Country@46dab859").toString());
		// } catch (Exception e) {
		// e.printStackTrace(); //To change body of catch statement use File |
		// Settings | File Templates.
		// }
		...
		//
		// PowerLawDistribution powerLawDistribution = new
		// PowerLawDistribution(10000, 30);
		// // ZipfDistribution zipfDistribution = new
		// PowerLawDistribution(1000,0.000001);
		//
		//
		// File file = new
		// File("/Users/pedro/Desktop/Resultados/distribution/dist.csv");
		//
	...
	// public static Map<String, Object> read_from_ColumnFamily(
	// Cassandra.Client client, String column_family, Object key) throws
	// Exception {
	//
	//
	...
	// SliceRange(ByteBuffer.wrap(getByteRepresentation("")),ByteBuffer.wrap(getByteRepresentation("")),false,100));
	//
	// List<ColumnOrSuperColumn> columnOrSuperColumns;
	//
	// columnOrSuperColumns = client.get_slice(key_bytes, columnPath,
	// slicePredicate, ConsistencyLevel.ONE);
	//
	// Map<String, Object> results = new TreeMap<String, Object>();
	//
	// for (ColumnOrSuperColumn columnOrSuperColumn : columnOrSuperColumns) {
	// results.put(new String(columnOrSuperColumn.getColumn().getName()),
	// columnOrSuperColumn.getColumn().getValue());
	// }
	//
	// return results;
	// }
	...
	public static void getData(String CF, String key)
	{
		...
		//
		// try {
		// System.out.println(read_from_ColumnFamily(client,CF ,
		// key).toString());
		// } catch (Exception e) {
		// e.printStackTrace(); //To change body of catch statement use File |
		// Settings | File Templates.
		...
		try
		{
			...
			for (int i = 0; i < TestClass.num_it; i++)
			{
				...
				PreparedStatement statement = con
						.prepareStatement("UPDATE ITEM SET i_stock = ((SELECT i_stock FROM ITEM WHERE i_id = 10) +1) WHERE i_id = 10");
				...
				//
				// PreparedStatement statement2 = con.prepareStatement
				// ("UPDATE ITEM SET i_stock = " + (stock+1) +
				// " WHERE i_id = 10");
				// statement2.execute();
...
// //As the the values for the buy confirm and buy request are not included,
// there is a second iteration to add the remaining value to the other options.
// for (int i = 0; i < 2; i++) {
...
// workload_values.put("search", prob + aggregated_probability + (remaining *
// (prob / 100d)));
// aggregated_probability += prob + (remaining * (prob / 100d));
//
...
// }
//
// //set buy options probabilities. If shopping cart is 2% and the buy request
// is 0,75% then of each cart there is a 0,41% probability.
...
// System.out.println("OP9 ShoppingCart:" + ((i9 / (operation_num * 1d)) *
// 100));
// System.out.println("OP6 CostumerRegis:" + ((i6 / (operation_num * 1d)) *
// 100));
...
// java.lang.String>> map = c.describe_keyspace("Keyspace1");
// //
...
//
//
// // byte[]utf8Bytes = new byte[]{-84, -19, 0, 5, 116, 0, 5, 48, 46, 48, 46,
// 48};
// //
// //
// // try {
...
// SliceRange("".getBytes(),"".getBytes(),false,((int)(100)));
...
// // krange.setEnd_key("");
// //
// // java.util.List<org.apache.cassandra.thrift.KeySlice> key_columns =
// c.get_range_slices("Tpcw", parent , slice_predicate,krange,
// ConsistencyLevel.QUORUM);
//
//
// // List<ColumnOrSuperColumn> key_columns = c.get_slice("Tpcw", "434324r" ,
// parent, slice_predicate, ConsistencyLevel.QUORUM);
...
// // fields.add("I_TITLE");
// // fields.add("I_STOCK");
// // Map<String, Map<String, Object>> items_info =
// DBinterface.getClient().rangeQuery("Items", fields, -1);
// //
// //
// // System.out.println("[INFO:] ITEMS COLLECTED FROM THE DATABASE: SIZE = " +
// items_info.size());
...
// if (decision_factor < 0.2) {
// parametros.put("CUSTOMER", nodeID.getId() + "." + private_id + "." +
// customer_id);
