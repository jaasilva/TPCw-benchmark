...
public class TPCWWorkloadGeneration implements WorkloadGeneratorInterface
{
	...
	String[] subjects = { "ARTS", "BIOGRAPHIES", "BUSINESS", "CHILDREN",
			...
			"REFERENCE", "RELIGION", "ROMANCE", "SELF-HELP", "SCIENCE-NATURE",
	...
	public TPCWWorkloadGeneration(ResultHandler handler, List<String> items,
			List<String> authors, Map<String, Double> workload,
	...
	public Operation getNextOperation()
	{
		...
		else
		{
			...
			else if (d < workload_values.get("search"))
			{
				...
				{
					String subject_field = author_names.get(rand
							.nextInt(author_names.size()));
				...
				{
					String subject_field = subjects[rand
			...
			else if (d < workload_values.get("register/login"))
			{
				...
				{
					parametros.put("CUSTOMER", nodeID.getId() + "."
	...
	public void printStats()
	{
		...
		if (operation_num == progressBar.getTop_limit())
		{
			...
			resultHandler.concurrent_countEvent("WORKLOAD", "PRODUCT DETAIL",
					i4);
			resultHandler.concurrent_countEvent("WORKLOAD", "SEARCH", i5);
			resultHandler.concurrent_countEvent("WORKLOAD",
					"COSTUMER REGISTRATION", i6);
			resultHandler
					.concurrent_countEvent("WORKLOAD", "ORDER INQUIRY", i7);
