...
public class ConsistencyWorkloadGenerator implements WorkloadGeneratorInterface
{
	...
	public ConsistencyWorkloadGenerator(List<String> items,
			ProbabilityDistribution distribution, String clientID,
	...
	public Operation getNextOperation()
	{
		...
		if (items_to_buy.isEmpty() && watching_item == false)
		{
			...
			if (debug_number > 9)
			{
				System.out.println("[WARN|" + private_id
						+ ":] SUSPECT NUMBER OF ITEMS BEIng ADDED TO CART");
			}
			debug_number = 0;
		}
		else
		{
			...
			if (watching_item)
			{
				...
				{
					System.out
							.println("[ERROR:] ITEM BEING WATCHED WAS NULL STOCK");
				...
				if (stock <= 0)
				{
					...
					if (stock < 0)
						System.out
								.println("[ERROR:] CANT RETRIEVE STOCK FOR ITEM : "
										+ last_item);
