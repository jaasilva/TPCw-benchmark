		...
		AbstractWorkloadGeneratorFactory
{
	 ...
	 */
	public ConsistencyTestWorkloadFactory(BenchmarkExecutor executor,
			String workload)
	{
		...
		else
		{
			...
			{
				System.out
						.println("[WARNING:] NO STOCK HANDLER CONFIGURATION DATA -> initialStock at 50 000 ");
				initial_stock = 50000;
			}
			else
			{
			...
			{
				mean_cart_items = Integer.parseInt(conf.get("mean_cart_items")
						.trim());
			}
	...
	public void init() throws Exception
	{
		...
		Operation op = new Operation("Get_Stock_And_Products_after_increment",
				param);
		...
		Map<String, Map<String, Object>> items_info = (Map<String, Map<String, Object>>) op
				.getResult();
		...
		{
			System.out
					.println("[INFO:] NO ITEMS  => BENCHMARK END, GOODBYE SIR!");
			System.exit(0);
		}
		...
		info.put("----", "----");
		String think_time = (BenchmarkMain.thinkTime == -1) ? "tpcw think time"
				: "user set: " + BenchmarkMain.thinkTime;
	...
	public WorkloadGeneratorInterface getClient()
	{
		...
		ConsistencyWorkloadGenerator client = new ConsistencyWorkloadGenerator(
				items_ids, distribution, nodeID.getId() + "." + client_number,
	...
	public void finishExecution(List<ResultHandler> collected_results)
	{
		...
		for (ResultHandler client : collected_results)
		{
			...
			if (!map.isEmpty())
			{
				...
				Map<String, Integer> partialBought = (Map<String, Integer>) client
						.getResulSet().get("bought");
		...
		DatabaseExecutorInterface database_client = this.databaseFactory
				.getDatabaseClient();
		...
		if (nodeID.isMaster())
		{
			...
			{
				int bought = BoughtItems.containsKey(item) ? BoughtItems
						.get(item) : 0;
				Results result = new Results(bought, items.get(item),
						nodeID.getId() + "");
				try
				{
	...
	public void consolidate() throws Exception
	{
		...
		{ // Master
			...
			DatabaseExecutorInterface databaseCrudClient = databaseFactory
					.getDatabaseClient();
			...
			Map<String, Map<String, Map<String, Object>>> result_info = (Map<String, Map<String, Map<String, Object>>>) op
					.getResult();
			...
			for (String clientID : result_info.keySet())
			{
				...
				Map<String, Map<String, Object>> item_info = result_info
						.get(clientID);
				// System.out.println("[INFO:] Items result size:" +
				// item_info.keySet().size());
				for (String itemID : item_info.keySet())
				{
					...
					if (!items_FinalInfo.containsKey(itemID))
					{
						...
						Object o = databaseCrudClient.read(itemID, "item",
								"I_STOCK", null);
						int stock = 0;
						if (o != null)
						{
						...
						{
							System.out
									.println("[ERROR:]ITEM CURRENT STOCK NOT FOUND");
						}

						if (added_stock.containsKey(itemID))
						{
							items_FinalInfo.put(itemID,
									new int[] { added_stock.get(itemID), 0,
											(int) stock });
						}
						else
						{
							items_with_no_added_stock++;
							items_FinalInfo.put(itemID, new int[] { 0, 0,
									(int) stock });
						}
					...
					for (String value_name : item_data.keySet())
					{
						...
						else if (value_name.equalsIgnoreCase("BOUGHT"))
						{
							...
							if (items_FinalInfo.get(itemID)[2] == 500000
									&& (current_b + read_b) != 0)
							{
			...
			System.out.println("[iNFO:]ITEMS WITH NO ADDED STOCK:"
					+ items_with_no_added_stock);
			...
			for (String it : items_FinalInfo.keySet())
			{
				...
				int stock = item_d[0] - item_d[2]; // (initial + added) minus
													// the stock still in the DB
													// = used stock
				...
				if (out_of_stock < 0)
				{
					System.out.println("Negative stock failure --  b: "
							+ item_d[1] + " || es:" + item_d[2]);
				}
				data.add(out_of_stock);
				total_out_stock += out_of_stock;

				globalResultHandler.recordData("BUYING RESULTS", item_id + "",
						data);
