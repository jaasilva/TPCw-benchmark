...
public class PowerLawDistribution implements ProbabilityDistribution
{
	...
	{
		return "Power Law Distribution"; // To change body of implemented
	...
	public void init(int numberElements, Map<String, Object> options)
	{
		...
		if (BenchmarkMain.distribution_factor != -1)
		{
			logger.warn("Power law factor set to user defined level: "
		...
		{
			System.out
					.println("[WARN:] ALPHA OPTION IS NOT DEFINED IN USED POWER LAW DISTRIBUTION. DEFAULT: 2");
		}
		else
		{
			...
			{
				alpha = default_alpha;
				logger.warn("Power law factor set to default due to error:"
						+ default_alpha);
			}
		}
	}
	...
	{
		return new PowerLawDistribution(size, alpha); // To change body of
														// implemented methods
														// use File | Settings |
														// File Templates.
	}
