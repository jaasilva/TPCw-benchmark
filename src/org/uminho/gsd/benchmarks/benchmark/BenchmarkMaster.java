...
public class BenchmarkMaster
{
	...
	public BenchmarkMaster(BenchmarkExecutor executor,
			Map<String, Object> slaves)
	{
	...
	public void run() throws Exception
	{
		...
		{
			SlaveHandler sh = new SlaveHandler(clientId, host,
					(String) slaves.get(host));
	...
	class SlaveHandler implements Runnable
	{
		...
		SlaveHandler(int clientId, String host, String port)
		{
			...
			do
			{
				...
				{
					System.out.println("Failed to conenct to " + host + ":"
							+ port);
			...
			try
			{
				...
				while (!terminated)
				{
					...
					if (message != null && message.equalsIgnoreCase("KILL"))
					{
						...
						Socket s = new Socket("localhost", 64446); // kill
																	// himself
